...
function boardCardDragStart(e,el){if(!unlocked){e.preventDefault();return}e.dataTransfer.setData('text/board-item',el.dataset.boardItem);el.classList.add('dragging')}
...
function boardCardDragOver(e,el){e.preventDefault();const item=e.dataTransfer.types.includes('text/board-item');if(!item)return;document.querySelectorAll('[style*="outline: 2px dashed"]').forEach(x=>x.style.outline='');el.style.outline='2px dashed var(--accent)'}
function boardCardDrop(e,el){e.preventDefault();e.stopPropagation();el.style.outline='';const src=e.dataTransfer.getData('text/board-item');if(!src||!unlocked)return;const targetGroup=el.dataset.boardGroup||el.closest('[data-board-group]')?.dataset.boardGroup;const target=el.dataset.boardItem;if(targetGroup){if(targetGroup===src)return;_manualBoardGroups[src]=targetGroup}else if(target){if(target===src)return;_manualBoardGroups[src]=target;_manualBoardGroups[target]=target}else return;saveBoardGroups();boardCardDragEnd();renderOutsourceFromCache()}
function moveOutOfGroup(e,item){e.stopPropagation();if(!unlocked)return;delete _manualBoardGroups[item];saveBoardGroups();renderOutsourceFromCache()}
...
function renderOutsourceFromCache(){
  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    const groups2=[];
    items.forEach(t=>{const mk=_manualBoardGroups[t['工作項目']];if(mk){const g=groups2.find(gr=>(gr.key||gr[0]['工作項目'])===mk);if(g){g.push(t)}else{const ng=[t];ng.key=mk;groups2.push(ng)}}else{const g=groups2.find(gr=>!gr.key&&sim(gr[0]['工作項目'],t['工作項目']));if(g)g.push(t);else groups2.push([t])}});
