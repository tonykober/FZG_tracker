...
function renderTimeline(){
  ...
  for(let d=1;d<=days;d++){const dow=new Date(y,m,d).getDay();if(dow===0||dow===6){const pos=((d-1)/days*100).toFixed(1);h+=`<div style="position:absolute;top:40px;bottom:0;left:calc(200px + (100% - 150px) * ${pos} / 100);width:calc((100% - 150px) / ${days});background:rgba(56,139,253,0.1);pointer-events:none"></div>`}}
  if(isThisMonth){const todayPos=((today.getDate()-0.5)/days*100).toFixed(1);h+=`<div style="position:absolute;top:40px;bottom:0;left:calc(200px + (100% - 150px) * ${todayPos} / 100);width:2px;background:var(--red);z-index:10;pointer-events:none;opacity:0.7"></div>`}
  h+='</div>';
    ...
    Object.entries(gGroups).sort((a,b)=>{const ai=tlSort2.indexOf(a[0]),bi=tlSort2.indexOf(b[0]);return(ai<0?999:ai)-(bi<0?999:bi)}).forEach(([owner,items])=>{
      ...
      tGroups.forEach(gr=>{
        if(gr.length===1){
          ...
          gr.forEach(t=>{let sd=parseDay(t['開始日']),ed=parseDay(t['截止日']);if(ed===null)ed=sd;if(sd===null||sd>days||ed<1||sd>ed)return;sd=Math.max(1,sd);ed=Math.min(days,ed);const color=(t['工作項目']||'').includes('請假')?'var(--red)':t['狀態']==='已完成'?'var(--green)':t['狀態']==='進行中'?'var(--yellow)':'var(--muted)';const l=((sd-1)/days*100).toFixed(1),w=((ed-sd+1)/days*100).toFixed(1);gh+=`<div data-group-item onclick="ganttRowClick(this,'${(t['工作項目']||'').replace(/'/g,"\\'")}')" style="display:flex;align-items:center;padding:2px 0;cursor:pointer"><div style="width:200px;flex-shrink:0;font-size:0.75rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-left:28px;color:var(--muted)">${t['工作項目']}</div><div style="flex:1;position:relative;height:12px;background:var(--bg);border-radius:3px"><div style="position:absolute;left:${l}%;width:${w}%;height:100%;background:${color};border-radius:3px;opacity:0.8"></div></div></div>`});
          gh+=`</div><div style="position:absolute;top:0;bottom:0;left:calc(200px + (100% - 150px) * ${gl} / 100 - 6px);width:calc((100% - 150px) * ${gw} / 100 + 12px);border:1px solid var(--border);border-radius:4px;pointer-events:none;z-index:0;background:rgba(88,166,255,0.15)"></div></div>`;
        }
      });
      gh+=`</div></div>`;
    });
    for(let d=1;d<=days;d++){const dow=new Date(y,m,d).getDay();if(dow===0||dow===6){const pos=((d-1)/days*100).toFixed(1);gh+=`<div style="position:absolute;top:40px;bottom:0;left:calc(200px + (100% - 150px) * ${pos} / 100);width:calc((100% - 150px) / ${days});background:rgba(56,139,253,0.1);pointer-events:none"></div>`}}
    if(isThisMonth2){const todayPos=((today2.getDate()-0.5)/days*100).toFixed(1);gh+=`<div style="position:absolute;top:40px;bottom:0;left:calc(200px + (100% - 150px) * ${todayPos} / 100);width:2px;background:var(--red);z-index:10;pointer-events:none;opacity:0.7"></div>`}
    gh+='</div></div>';
