...
function boardCardDrop(e,el){e.preventDefault();e.stopPropagation();el.style.outline='';const src=e.dataTransfer.getData('text/board-item');if(!src||!unlocked)return;const target=el.dataset.boardItem||el.dataset.boardGroup;if(!target||target===src)return;if(el.dataset.boardGroup){_manualBoardGroups[src]=target}else{_manualBoardGroups[src]=target;_manualBoardGroups[target]=target}saveBoardGroups();boardCardDragEnd();renderOutsourceFromCache()}
