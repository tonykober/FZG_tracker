  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    groups2.forEach(gr=>{
      if(gr.length===1){
        ...
        c+=`<div class="card" data-board-item="${t['工作項目'].replace(/"/g,'&quot;')}" draggable="true" ondragstart="boardCardDragStart(event,this)" ondragend="boardCardDragEnd()" ondragover="boardCardDragOver(event,this)" ondragleave="this.style.outline=''" ondrop="boardCardDrop(event,this)" style="cursor:grab"><div class="name" style="color:${statusColor}">${statusIcon} ${t['工作項目']}</div><div class="meta"><span>${t['狀態']}${t['備註']?' '+t['備註']:''}</span><span>${t['開始日']?t['開始日'].substring(0,10):''}${t['開始日']&&t['截止日']?' ~ ':''}${t['截止日']?t['截止日'].substring(0,10):''}</span></div></div>`;
      }else{
        ...
        c+=`<div class="card" data-board-group="${grKey.replace(/"/g,'&quot;')}" ondragover="boardCardDragOver(event,this)" ondragleave="this.style.outline=''" ondrop="boardCardDrop(event,this)" style="cursor:default"><div class="name" data-board-group="${grKey.replace(/"/g,'&quot;')}" onclick="toggleOutsourceBoardGroup(this)" style="cursor:pointer"><span>${_collapsedOutsourceBoardGroups.has(grKey)?'▼':'▶'}</span> <span class="edit-ctrl" onclick="event.stopPropagation();editGroupName(this,'${grKey.replace(/'/g,"\\'")}')" style="cursor:pointer">✏️</span><span class="view-ctrl">📁</span> ${grName} (${gr.length})</div><div${_collapsedOutsourceBoardGroups.has(grKey)?'':' style="display:none"'}>`;
        gr.forEach(t=>{const statusIcon=t['狀態']==='已完成'?'✅':t['狀態']==='進行中'?'🔄':'📝';const statusColor=t['工作項目'].includes('請假')?'var(--red)':t['狀態']==='已完成'?'var(--green)':t['狀態']==='進行中'?'var(--yellow)':'var(--muted)';c+=`<div data-board-item="${t['工作項目'].replace(/"/g,'&quot;')}" draggable="true" ondragstart="boardCardDragStart(event,this)" ondragend="boardCardDragEnd()" style="padding:2px 0;font-size:0.85em;color:${statusColor};cursor:grab">${statusIcon} ${t['工作項目']} <span style="color:var(--muted)">${t['開始日']?t['開始日'].substring(0,10):''}</span><span class="edit-ctrl" onclick="moveOutOfGroup(event,'${t['工作項目'].replace(/'/g,"\\'")}')" style="cursor:pointer;margin-left:4px">⤴️</span></div>`});
        c+=`</div></div>`;
      }
    });
    c+=`</div></div>`;
