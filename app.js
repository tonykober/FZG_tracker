...
function getManualGroupsKey(){return 'fzg_manual_groups_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1)}
let _groupNames={};
let _manualGroups={};
function loadGroupNames(){_groupNames=JSON.parse(localStorage.getItem(getGroupNamesKey())||'{}');_manualGroups=JSON.parse(localStorage.getItem(getManualGroupsKey())||'{}')}
...
let _dragItem=null;
function itemDragStart(e,el){if(!unlocked){e.preventDefault();return}_dragItem=el.dataset.item;e.dataTransfer.setData('text/item',_dragItem);el.classList.add('dragging')}
function itemDragEnd(){_dragItem=null;document.querySelectorAll('.dragging,.drag-over-top,.drag-over-bottom,.drag-target').forEach(x=>x.classList.remove('dragging','drag-over-top','drag-over-bottom','drag-target'))}
function groupDragOver(e,el){e.preventDefault();if(!_dragItem)return;document.querySelectorAll('.drag-over-top,.drag-over-bottom,.drag-target').forEach(x=>x.classList.remove('drag-over-top','drag-over-bottom','drag-target'));el.classList.add('drag-target')}
function groupDrop(e,el){e.preventDefault();e.stopPropagation();document.querySelectorAll('.drag-target').forEach(x=>x.classList.remove('drag-target'));if(!_dragItem||!unlocked)return;const targetGroup=el.closest('[data-group]')?.dataset.group;if(!targetGroup||targetGroup===_dragItem)return;_manualGroups[_dragItem]=targetGroup;saveManualGroups();itemDragEnd();renderOutsourceFromCache()}
function itemTargetDragOver(e,el){e.preventDefault();if(!_dragItem||el.dataset.item===_dragItem)return;document.querySelectorAll('.drag-target,.drag-over-top,.drag-over-bottom').forEach(x=>x.classList.remove('drag-target','drag-over-top','drag-over-bottom'));if(_manualGroups[_dragItem]){const rect=el.getBoundingClientRect();el.classList.add(e.clientY<rect.top+rect.height/2?'drag-over-top':'drag-over-bottom')}else{el.classList.add('drag-target')}}
function itemTargetDrop(e,el){e.preventDefault();e.stopPropagation();document.querySelectorAll('.drag-target,.drag-over-top,.drag-over-bottom').forEach(x=>x.classList.remove('drag-target','drag-over-top','drag-over-bottom'));if(!_dragItem||!unlocked)return;const target=el.dataset.item;if(!target||target===_dragItem)return;if(_manualGroups[_dragItem]){delete _manualGroups[_dragItem];saveManualGroups();itemDragEnd();renderOutsourceFromCache()}else{_manualGroups[_dragItem]=target;_manualGroups[target]=target;saveManualGroups();itemDragEnd();renderOutsourceFromCache()}}
function removeFromGroup(e,el){e.preventDefault();e.stopPropagation();if(!_dragItem||!unlocked)return;delete _manualGroups[_dragItem];saveManualGroups();itemDragEnd();renderOutsourceFromCache()}
function saveManualGroups(){localStorage.setItem(getManualGroupsKey(),JSON.stringify(_manualGroups));fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify({action:'saveNote',month:'manual_groups_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(_manualGroups)}),mode:'no-cors'})}
...
function renderOutsourceFromCache(){
  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    // Group similar items with manual overrides
    const sim=window._isSimilar||(()=>false);
    const groups2=[];
    items.forEach(t=>{const manualKey=_manualGroups[t['工作項目']];if(manualKey){const g=groups2.find(gr=>gr[0]['工作項目']===manualKey||gr.key===manualKey);if(g){g.push(t)}else{const ng=[t];ng.key=manualKey;groups2.push(ng)}}else{const g=groups2.find(gr=>!gr.key&&sim(gr[0]['工作項目'],t['工作項目']));if(g)g.push(t);else groups2.push([t])}});
    groups2.forEach(gr=>{
      if(gr.length===1){
  ...
  if(outsourceMode==='gantt'){
    ...
    Object.entries(gGroups).sort((a,b)=>{const ai=tlSort2.indexOf(a[0]),bi=tlSort2.indexOf(b[0]);return(ai<0?999:ai)-(bi<0?999:bi)}).forEach(([owner,items])=>{
      ...
      // Group similar items for gantt with manual overrides
      const sim=window._isSimilar||(()=>false);
      const tGroups=[];items.forEach(t=>{const manualKey=_manualGroups[t['工作項目']];if(manualKey){const g=tGroups.find(gr=>gr[0]['工作項目']===manualKey||gr.key===manualKey);if(g){g.push(t)}else{const ng=[t];ng.key=manualKey;tGroups.push(ng)}}else{const g=tGroups.find(gr=>!gr.key&&sim(gr[0]['工作項目'],t['工作項目']));if(g)g.push(t);else tGroups.push([t])}});
      ...
      tGroups.forEach(gr=>{
        if(gr.length===1){
          ...
          const l=((sd-1)/days*100).toFixed(1),w=((ed-sd+1)/days*100).toFixed(1);
          gh+=`<div data-item="${(t['工作項目']||'').replace(/"/g,'&quot;')}" draggable="true" ondragstart="itemDragStart(event,this)" ondragend="itemDragEnd()" ondragover="itemTargetDragOver(event,this)" ondragleave="this.classList.remove('drag-target')" ondrop="itemTargetDrop(event,this)" onclick="ganttRowClick(this,'${(t['工作項目']||'').replace(/'/g,"\\'")}')" style="display:flex;align-items:center;padding:4px 0;cursor:grab"><div style="width:200px;flex-shrink:0;font-size:0.875rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-left:12px"><span style="display:inline-block;width:16px"></span>${t['工作項目']}</div><div style="flex:1;position:relative;height:16px;background:var(--bg);border-radius:3px"><div style="position:absolute;left:${l}%;width:${w}%;height:100%;background:${color};border-radius:3px;opacity:0.8"></div></div></div>`;
        }else{
          const shortest=gr.reduce((a,b)=>a['工作項目'].length<=b['工作項目'].length?a:b)['工作項目'];
