...
function renderBoard(){
  ...
  const cardHtml=(items)=>items.map(t=>{
    ...
    return `<div class="card" data-idx="${idx}" ondragover="taskDragOver(event,this)" ondragleave="this.classList.remove('drag-over-top','drag-over-bottom')" ondrop="taskDrop(event,${idx},this)" style="${_dbg}">
      ...
      const _subCol=JSON.parse(localStorage.getItem('fzg_sub_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1))||'[]');const _subHidden=_subCol.includes(t['任務名稱']);
      ${children.length?'<div class="subtasks" onclick="toggleSub(this,event)" style="cursor:pointer"><span style="font-size:0.75rem">'+(_subHidden?'▶':'▼')+'</span> 子任務：'+childDone+'/'+children.length+'<div style="margin-top:4px'+(_subHidden?';display:none':'')+'">'+children.map(c=>{
