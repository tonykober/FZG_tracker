...
function saveNote(month,text){const url=SCRIPT_URL+'?action=saveNote&month='+encodeURIComponent(month)+'&text='+encodeURIComponent(text);setSyncStatus('🔄 同步中...','var(--yellow)');if(url.length<2000)return fetch(url).then(()=>setSyncStatus('✅ 已同步','var(--green)')).catch(()=>setSyncStatus('❌ 同步失敗','var(--red)'));navigator.sendBeacon(SCRIPT_URL,JSON.stringify({action:'saveNote',month:month,text:text}));setSyncStatus('✅ 已同步','var(--green)');return Promise.resolve()}
...
function boardCardDrop(e,el){e.preventDefault();e.stopPropagation();el.style.outline='';const src=e.dataTransfer.getData('text/board-item');if(!src||!unlocked)return;const groupEl=el.closest('[data-board-group]');const targetGroup=groupEl?.dataset.boardGroup||el.dataset.boardGroup;const target=el.dataset.boardItem;if(!targetGroup&&!target)return;if(targetGroup){if(targetGroup===src)return;_manualBoardGroups[src]=targetGroup;const gid=window._getTaskId||(t=>t['工作項目']);outsourceTasks.forEach(t=>{const id=gid(t);if(_manualBoardGroups[id]===targetGroup||id===targetGroup)_manualBoardGroups[id]=targetGroup})}else if(target){if(target===src)return;_manualBoardGroups[src]=target;_manualBoardGroups[target]=target;const gid=window._getTaskId||(t=>t['工作項目']);const sim=window._isSimilar||(()=>false);const srcTask=outsourceTasks.find(t=>gid(t)===src);const tgtTask=outsourceTasks.find(t=>gid(t)===target);if(srcTask&&tgtTask){outsourceTasks.forEach(t=>{const id=gid(t);if(id!==src&&id!==target&&!_manualBoardGroups[id]&&(sim(t['工作項目'],srcTask['工作項目'])||sim(t['工作項目'],tgtTask['工作項目']))){_manualBoardGroups[id]='__independent__'}})}}saveBoardGroups();boardCardDragEnd();renderOutsourceFromCache()}
function moveOutOfGroup(e,itemId){e.stopPropagation();if(!unlocked)return;const grKey=_manualBoardGroups[itemId];if(grKey&&grKey!=='__independent__'){const gid=window._getTaskId||(t=>t['工作項目']);outsourceTasks.forEach(t=>{const id=gid(t);if(id!==itemId&&(_manualBoardGroups[id]===grKey||id===grKey))_manualBoardGroups[id]=grKey})}else{const gid=window._getTaskId||(t=>t['工作項目']);const sim=window._isSimilar||(()=>false);const item=outsourceTasks.find(t=>gid(t)===itemId);if(item){const grMembers=outsourceTasks.filter(t=>gid(t)!==itemId&&sim(t['工作項目'],item['工作項目'])&&_manualBoardGroups[gid(t)]!=='__independent__');if(grMembers.length>1){const key=gid(grMembers[0]);grMembers.forEach(t=>{_manualBoardGroups[gid(t)]=key})}}}_manualBoardGroups[itemId]='__independent__';saveBoardGroups();renderOutsourceFromCache()}
