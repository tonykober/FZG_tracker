...
function moveBoardItem(owner,key,dir,e){e.stopPropagation();if(!unlocked)return;const sort=JSON.parse(localStorage.getItem(getBoardItemSortKey())||'{}');const list=sort[owner]||[];const pos=list.indexOf(key);if(pos<0){list.push(key);sort[owner]=list;return}const np=pos+dir;if(np<0||np>=list.length)return;list.splice(pos,1);list.splice(np,0,key);sort[owner]=list;localStorage.setItem(getBoardItemSortKey(),JSON.stringify(sort));fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify({action:'saveNote',month:'board_item_sort_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(sort)}),mode:'no-cors'});renderOutsourceFromCache()}
...
function boardCardDrop(e,el){e.preventDefault();e.stopPropagation();el.style.outline='';const src=e.dataTransfer.getData('text/board-item');if(!src||!unlocked)return;const targetGroup=el.dataset.boardGroup||el.closest('[data-board-group]')?.dataset.boardGroup;const target=el.dataset.boardItem;if(!targetGroup&&!target)return;if(targetGroup){if(targetGroup===src)return;_manualBoardGroups[src]=targetGroup;if(!_manualBoardGroups[targetGroup])_manualBoardGroups[targetGroup]=targetGroup}else if(target){if(target===src)return;_manualBoardGroups[src]=target;_manualBoardGroups[target]=target}saveBoardGroups();boardCardDragEnd();renderOutsourceFromCache()}
function moveOutOfGroup(e,item){e.stopPropagation();if(!unlocked)return;const grKey=_manualBoardGroups[item];if(grKey){delete _manualBoardGroups[item]}else{const sim=window._isSimilar||(()=>false);outsourceTasks.forEach(t=>{if(t['工作項目']!==item&&sim(t['工作項目'],item)){if(!_manualBoardGroups[t['工作項目']])_manualBoardGroups[t['工作項目']]=t['工作項目']}});_manualBoardGroups[item]='__independent__'}saveBoardGroups();renderOutsourceFromCache()}
...
function renderOutsourceFromCache(){
  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    const groups2=[];
    items.forEach(t=>{const mk=_manualBoardGroups[t['工作項目']];if(mk==='__independent__'){groups2.push([t]);return}if(mk){const g=groups2.find(gr=>(gr.key||gr[0]['工作項目'])===mk);if(g){g.push(t)}else{const ng=[t];ng.key=mk;groups2.push(ng)}}else{const g=groups2.find(gr=>!gr.key&&sim(gr[0]['工作項目'],t['工作項目'])&&_manualBoardGroups[gr[0]['工作項目']]!=='__independent__');if(g)g.push(t);else groups2.push([t])}});
    const boardSort=JSON.parse(localStorage.getItem(getBoardItemSortKey())||'{}');
    const ownerOrder=boardSort[owner]||[];
    groups2.sort((a,b)=>{const ak=a.key||a[0]['工作項目'],bk=b.key||b[0]['工作項目'];const ai=ownerOrder.indexOf(ak),bi=ownerOrder.indexOf(bk);return(ai<0?999:ai)-(bi<0?999:bi)});
    // Register keys for sort
    const sortKeys=groups2.map(gr=>gr.key||gr[0]['工作項目']);if(!ownerOrder.length){boardSort[owner]=sortKeys;localStorage.setItem(getBoardItemSortKey(),JSON.stringify(boardSort))}
    groups2.forEach(gr=>{
      if(gr.length===1){
        ...
        c+=`<div class="card" data-board-item="${t['工作項目'].replace(/"/g,'&quot;')}" draggable="true" ondragstart="boardCardDragStart(event,this)" ondragend="boardCardDragEnd()" ondragover="boardCardDragOver(event,this)" ondragleave="this.style.outline=''" ondrop="boardCardDrop(event,this)" style="cursor:grab"><div class="name" style="color:${statusColor};display:flex;align-items:center">${statusIcon} ${t['工作項目']}<span class="edit-ctrl" style="margin-left:auto;display:flex;gap:2px"><span onclick="moveBoardItem('${owner.replace(/'/g,"\\'")}','${t['工作項目'].replace(/'/g,"\\'")}', -1, event)" style="cursor:pointer">▲</span><span onclick="moveBoardItem('${owner.replace(/'/g,"\\'")}','${t['工作項目'].replace(/'/g,"\\'")}', 1, event)" style="cursor:pointer">▼</span></span></div><div class="meta"><span>${t['狀態']}${t['備註']?' '+t['備註']:''}</span><span>${t['開始日']?t['開始日'].substring(0,10):''}${t['開始日']&&t['截止日']?' ~ ':''}${t['截止日']?t['截止日'].substring(0,10):''}</span></div></div>`;
      }else{
        ...
        gr.forEach(t=>{const statusIcon=t['狀態']==='已完成'?'✅':t['狀態']==='進行中'?'🔄':'📝';const statusColor=t['工作項目'].includes('請假')?'var(--red)':t['狀態']==='已完成'?'var(--green)':t['狀態']==='進行中'?'var(--yellow)':'var(--muted)';c+=`<div style="padding:2px 0;font-size:0.85em;color:${statusColor};display:flex;align-items:center">${statusIcon} ${t['工作項目']} <span style="color:var(--muted);margin-left:4px">${t['開始日']?t['開始日'].substring(0,10):''}</span><span class="edit-ctrl" onclick="moveOutOfGroup(event,'${t['工作項目'].replace(/'/g,"\\'")}')" style="cursor:pointer;margin-left:auto">⤴️</span></div>`});
        c+=`</div></div>`;
      }
    });
    c+=`</div></div>`;
  ...
  if(outsourceMode==='gantt'){
    ...
    Object.entries(gGroups).sort((a,b)=>{const ai=tlSort2.indexOf(a[0]),bi=tlSort2.indexOf(b[0]);return(ai<0?999:ai)-(bi<0?999:bi)}).forEach(([owner,items])=>{
      ...
      const sim=window._isSimilar||(()=>false);
      const tGroups=[];items.forEach(t=>{const mk=_manualBoardGroups[t['工作項目']];if(mk==='__independent__'){tGroups.push([t]);return}if(mk){const g=tGroups.find(gr=>(gr.key||gr[0]['工作項目'])===mk);if(g){g.push(t)}else{const ng=[t];ng.key=mk;tGroups.push(ng)}}else{const g=tGroups.find(gr=>!gr.key&&sim(gr[0]['工作項目'],t['工作項目'])&&_manualBoardGroups[gr[0]['工作項目']]!=='__independent__');if(g)g.push(t);else tGroups.push([t])}});
