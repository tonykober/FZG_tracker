...
function moveBoardItem(owner,key,dir,e){e.stopPropagation();if(!unlocked)return;const sort=JSON.parse(localStorage.getItem(getBoardItemSortKey())||'{}');let list=sort[owner]||[];if(!list.length){const gid=window._getTaskId||(t=>t['工作項目']);const sim=window._isSimilar||(()=>false);const ownerItems=outsourceTasks.filter(t=>(t['負責人']||'未指派')===owner);const groups3=[];ownerItems.forEach(t=>{const mk=_manualBoardGroups[gid(t)];if(mk==='__independent__'){groups3.push(gid(t));return}if(mk){if(!groups3.includes(mk))groups3.push(mk)}else{const existing=groups3.find(k=>typeof k==='string'&&ownerItems.some(o=>gid(o)===k&&sim(o['工作項目'],t['工作項目'])));if(!existing)groups3.push(gid(t))}});list=groups3;sort[owner]=list}const pos=list.indexOf(key);if(pos<0)return;const np=pos+dir;if(np<0||np>=list.length)return;list.splice(pos,1);list.splice(np,0,key);sort[owner]=list;localStorage.setItem(getBoardItemSortKey(),JSON.stringify(sort));fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify({action:'saveNote',month:'board_item_sort_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(sort)}),mode:'no-cors'});renderOutsourceFromCache()}
  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    groups2.forEach(gr=>{
      if(gr.length===1){
        ...
        const grKey=gr.key||gid(gr[0]);const grName=getGroupDisplayName(shortest,gr);
