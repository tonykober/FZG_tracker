    ...
    Object.entries(gGroups).sort((a,b)=>{const ai=tlSort2.indexOf(a[0]),bi=tlSort2.indexOf(b[0]);return(ai<0?999:ai)-(bi<0?999:bi)}).forEach(([owner,items])=>{
      ...
      tGroups.forEach(gr=>{
        if(gr.length===1){
          ...
          gh+=`<div data-group="${shortest.replace(/"/g,'&quot;')}" style="margin:2px 0"><div style="display:flex;align-items:center;padding:4px 0"><div style="width:150px;flex-shrink:0;font-size:0.875rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-left:12px"><span onclick="event.stopPropagation();toggleOutsourceGroup(this)" style="display:inline-block;width:16px;text-align:center;cursor:pointer">${grCollapsed?'▶':'▼'}</span><span onclick="ganttRowClick(this.closest('[data-group]').querySelector('[style*=flex]'),'${shortest.replace(/'/g,"\\'")}')" style="cursor:pointer">📁 ${shortest} (${gr.length})</span></div><div style="flex:1;position:relative;height:16px;background:var(--bg);border-radius:3px"><div style="position:absolute;left:${gl}%;width:${gw}%;height:100%;background:var(--accent);border-radius:3px;opacity:0.4"></div></div></div><div${grCollapsed?' style="display:none"':''}>`;
          gr.forEach(t=>{let sd=parseDay(t['開始日']),ed=parseDay(t['截止日']);if(ed===null)ed=sd;if(sd===null||sd>days||ed<1||sd>ed)return;sd=Math.max(1,sd);ed=Math.min(days,ed);const color=(t['工作項目']||'').includes('請假')?'var(--red)':t['狀態']==='已完成'?'var(--green)':t['狀態']==='進行中'?'var(--yellow)':'var(--muted)';const l=((sd-1)/days*100).toFixed(1),w=((ed-sd+1)/days*100).toFixed(1);gh+=`<div onclick="ganttRowClick(this,'${(t['工作項目']||'').replace(/'/g,"\\'")}')" style="display:flex;align-items:center;padding:2px 0;cursor:pointer"><div style="width:150px;flex-shrink:0;font-size:0.75rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-left:28px;color:var(--muted)">${t['工作項目']}</div><div style="flex:1;position:relative;height:12px;background:var(--bg);border-radius:3px"><div style="position:absolute;left:${l}%;width:${w}%;height:100%;background:${color};border-radius:3px;opacity:0.8"></div></div></div>`});
          gh+=`</div></div>`;
        }
      });
      gh+=`</div></div>`;
