...
function tlTaskDrop(e,el){e.preventDefault();e.stopPropagation();document.querySelectorAll('.drag-over-top,.drag-over-bottom').forEach(x=>x.classList.remove('drag-over-top','drag-over-bottom'));if(!_tlTaskDrag||!unlocked||el.dataset.task===_tlTaskDrag)return;const sort=JSON.parse(localStorage.getItem(getTlTaskSortKey())||'{}');const src=_tlTaskDrag,tgt=el.dataset.task;const srcTask=tasks.find(t=>t['任務名稱']===src),tgtTask=tasks.find(t=>t['任務名稱']===tgt);if(!srcTask||!tgtTask||(srcTask['負責人']||'')!==(tgtTask['負責人']||''))return;const owner=srcTask['負責人']||'未指派';const ownerTasks=tasks.filter(t=>(t['負責人']||'未指派')===owner&&!t['父任務']).map(t=>t['任務名稱']);const from=ownerTasks.indexOf(src);if(from>=0)ownerTasks.splice(from,1);const to=ownerTasks.indexOf(tgt);const rect=el.getBoundingClientRect();ownerTasks.splice(e.clientY<rect.top+rect.height/2?to:to+1,0,src);sort[owner]=ownerTasks;localStorage.setItem(getTlTaskSortKey(),JSON.stringify(sort));fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify({action:'saveNote',month:'timeline_task_sort_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(sort)}),mode:'no-cors'});tlTaskDragEnd();render()}
