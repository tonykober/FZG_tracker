...
function renderBoard(){
  ...
  const cardHtml=(items)=>items.map(t=>{
    ...
    return `<div class="card" draggable="true" ondragstart="drag(event,${idx})" style="position:relative;${_dbg}"><span class="edit-ctrl" onclick="quickDelete(${idx},event)" style="position:absolute;top:4px;right:4px;cursor:pointer;font-size:0.75rem;background:var(--red);color:#fff;border-radius:4px;padding:2px 6px">✕</span>
      ...
      <div class="edit-ctrl" style="display:flex;gap:4px;margin-top:4px"><span onclick="event.stopPropagation();openModal(${idx})" style="font-size:0.75rem;background:#4caf50;color:#fff;border-radius:4px;padding:2px 6px;cursor:pointer">編輯</span>${canAddSub?`<span onclick="event.stopPropagation();openModalWithParent('${t['任務名稱'].replace(/'/g,"\\'")}')" style="font-size:0.75rem;background:var(--accent);color:#fff;border-radius:4px;padding:2px 6px;cursor:pointer">+子任務</span>`:''}</div>
      <div class="card-body">
