...
function itemDragEnd(){_dragItem=null;document.querySelectorAll('.dragging,.drag-over-top,.drag-over-bottom').forEach(x=>x.classList.remove('dragging','drag-over-top','drag-over-bottom'))}
function groupDragOver(e,el){e.preventDefault();if(!_dragItem)return;document.querySelectorAll('.drag-over-top,.drag-over-bottom').forEach(x=>x.classList.remove('drag-over-top','drag-over-bottom'));el.classList.add('drag-over-top')}
function groupDrop(e,el){e.preventDefault();e.stopPropagation();document.querySelectorAll('.drag-over-top,.drag-over-bottom').forEach(x=>x.classList.remove('drag-over-top','drag-over-bottom'));if(!_dragItem||!unlocked)return;const targetGroup=el.closest('[data-group]')?.dataset.group;if(!targetGroup)return;_manualGroups[_dragItem]=targetGroup;localStorage.setItem(getManualGroupsKey(),JSON.stringify(_manualGroups));fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify({action:'saveNote',month:'manual_groups_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(_manualGroups)}),mode:'no-cors'});itemDragEnd();renderOutsourceFromCache()}
