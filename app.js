...
function renderBoard(){
  ...
  const cardHtml=(items)=>items.map(t=>{
    ...
    return `<div class="card" draggable="true" ondragstart="drag(event,${idx})" style="${_dbg}">
      <div class="name" onclick="event.stopPropagation();var b=this.closest('.card').querySelector('.card-body');b.style.display=b.style.display==='none'?'block':'none';this.querySelector('span').textContent=b.style.display==='none'?'▶':'▼'" style="cursor:pointer"><span style="font-size:0.7em;margin-right:4px">▼</span>${pClass?'<span class="priority-dot '+pClass+'"></span>':''}${t['任務名稱']}</div>
      <div class="edit-ctrl" style="display:flex;gap:4px;margin-top:4px;align-items:center"><span onclick="event.stopPropagation();openModal(${idx})" style="font-size:0.75rem;background:#4caf50;color:#fff;border-radius:4px;padding:2px 6px;cursor:pointer">編輯</span>${canAddSub?`<span onclick="event.stopPropagation();openModalWithParent('${t['任務名稱'].replace(/'/g,"\\'")}')" style="font-size:0.75rem;background:var(--accent);color:#fff;border-radius:4px;padding:2px 6px;cursor:pointer">+子任務</span>`:''}<span onclick="quickDelete(${idx},event)" style="margin-left:auto;cursor:pointer;font-size:0.75rem;background:var(--red);color:#fff;border-radius:4px;padding:2px 6px">✕</span></div>
      <div class="card-body">
