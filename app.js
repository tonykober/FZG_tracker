...
function itemTargetDragOver(e,el){e.preventDefault();if(!_dragItem||el.dataset.item===_dragItem)return;document.querySelectorAll('.drag-target').forEach(x=>x.classList.remove('drag-target'));el.classList.add('drag-target')}
function itemTargetDrop(e,el){e.preventDefault();e.stopPropagation();document.querySelectorAll('.drag-target').forEach(x=>x.classList.remove('drag-target'));if(!_dragItem||!unlocked)return;const target=el.dataset.item;if(!target||target===_dragItem)return;_manualGroups[_dragItem]=target;_manualGroups[target]=target;saveManualGroups();itemDragEnd();renderOutsourceFromCache()}
