...
function toggleStatus(idx,e){
  ...
  t['狀態']=next;
  fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:idx,name:t['任務名稱'],owner:t['負責人'],status:next,progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']}),});
  render();
}
function inlineEdit(idx,field,e){
  ...
  m.querySelector('#ie-ok').onclick=()=>{
    ...
    fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:idx,name:t['任務名稱'],owner:t['負責人'],status:t['狀態'],progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']}),});
    m.remove();render();
  };
...
function quickDelete(idx,e){
  ...
  children.forEach(c=>{
    ...
    if(!parentOfDeleted){
      c['父任務']='';
      fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:ci,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:'',hours:c['工時'],comment:c['評論']}),});
    } else {
      c['父任務']=parentOfDeleted;
      fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:ci,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:parentOfDeleted,hours:c['工時'],comment:c['評論']}),});
    }
  ...
  fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'delete',row:idx}),});
...
function saveOwnerSort(status,sortArray){
  fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'saveNote',month:'owner_sort_'+status+'_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(sortArray)}),});
}
...
async function submitTask(){
  ...
  try{
    ...
    if(data.action==='update'){
      ...
      if(oldName!==data.name){tasks.filter(c=>c['父任務']===oldName).forEach(c=>{c['父任務']=data.name;const ci=tasks.indexOf(c);fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:ci,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:data.name,hours:c['工時'],comment:c['評論']}),})})}
    }else{
...
function deleteTask(idx){
  ...
  fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'delete',row:row}),});
...
function boardCardDrop(e,el){e.preventDefault();e.stopPropagation();el.style.outline='';const src=e.dataTransfer.getData('text/board-item');if(!src||!unlocked)return;const groupEl=el.closest('[data-board-group]');const targetGroup=groupEl?.dataset.boardGroup||el.dataset.boardGroup;const target=el.dataset.boardItem;if(!targetGroup&&!target)return;if(targetGroup){if(targetGroup===src)return;if(!_groupNames[targetGroup]){const grEl=document.querySelector('[data-board-group="'+targetGroup+'"] .name span:last-child');if(grEl)_groupNames[targetGroup]=grEl.textContent.replace(/\s*\(\d+\)$/,'').trim()}_manualBoardGroups[src]=targetGroup;if(!_manualBoardGroups[targetGroup])_manualBoardGroups[targetGroup]=targetGroup}else if(target){if(target===src)return;_manualBoardGroups[src]=target;_manualBoardGroups[target]=target}saveBoardGroups();boardCardDragEnd();renderOutsourceFromCache()}
function moveOutOfGroup(e,itemId){e.stopPropagation();if(!unlocked)return;const grKey=_manualBoardGroups[itemId];if(grKey&&grKey!=='__independent__'){_manualBoardGroups[itemId]='__independent__'}else{const gid=window._getTaskId||(t=>t['工作項目']);const sim=window._isSimilar||(()=>false);const item=outsourceTasks.find(t=>gid(t)===itemId);if(item){outsourceTasks.forEach(t=>{const tid=gid(t);if(tid!==itemId&&sim(t['工作項目'],item['工作項目'])&&_manualBoardGroups[tid]!=='__independent__'){if(!_manualBoardGroups[tid])_manualBoardGroups[tid]=tid}})}_manualBoardGroups[itemId]='__independent__'}saveBoardGroups();renderOutsourceFromCache()}
let _collapsedOutsourceBoardGroups=new Set();
function toggleOutsourceGroup(el){var wrapper=el.closest('[data-group]');var d=wrapper.children[1];d.style.display=d.style.display==='none'?'block':'none';el.textContent=d.style.display==='none'?'▶':'▼';var name=wrapper.dataset.group;if(!name)return;if(d.style.display!=='none')_collapsedOutsourceGroups.add(name);else _collapsedOutsourceGroups.delete(name);if(!unlocked)return;localStorage.setItem(getOutsourceGroupCollapseKey(),JSON.stringify([..._collapsedOutsourceGroups]));fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'saveNote',month:'expanded_outsource_groups_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify([..._collapsedOutsourceGroups])}),})}
function toggleOutsourceBoardGroup(el){var d=el.nextElementSibling;d.style.display=d.style.display==='none'?'block':'none';el.querySelector('span').textContent=d.style.display==='none'?'▶':'▼';var name=el.dataset.boardGroup;if(!name)return;if(d.style.display!=='none')_collapsedOutsourceBoardGroups.add(name);else _collapsedOutsourceBoardGroups.delete(name);if(!unlocked)return;localStorage.setItem(getOutsourceBoardGroupCollapseKey(),JSON.stringify([..._collapsedOutsourceBoardGroups]));fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'saveNote',month:'expanded_outsource_board_groups_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify([..._collapsedOutsourceBoardGroups])}),})}
...
function renderBoard(){
  ...
  const unHtml=unassigned.length?`<div style="margin-bottom:8px"><div onclick="var d=this.nextElementSibling;d.style.display=d.style.display==='none'?'block':'none';this.querySelector('.tog').textContent=d.style.display==='none'?'▶':'▼'" style="color:var(--red);padding:4px 0;border-bottom:1px solid var(--red);margin-bottom:4px;cursor:pointer"><span class="tog">▶</span> ⚠️ 有 ${unassigned.length} 筆任務資訊不全</div><div style="display:none">${unassigned.map(t=>{const idx=tasks.indexOf(t);return `<div class="card" style="display:flex;align-items:center;gap:8px"><span>${t['任務名稱']}</span><span class="edit-ctrl" style="margin-left:auto;display:flex;gap:4px"><span onclick="openModal(${idx})" style="cursor:pointer">✏️</span><span onclick="deleteTask(${idx})" style="cursor:pointer;color:var(--red)">🗑️</span></span></div>`}).join('')}</div></div>`:'';
  ...
  const unCol=unHtml?`<div class="column" style="border-color:var(--red)"><h3 onclick="var d=this.nextElementSibling;d.style.display=d.style.display==='none'?'block':'none';this.querySelector('.tog').textContent=d.style.display==='none'?'▶':'▼'" style="color:var(--red);cursor:pointer"><span class="tog">▼</span> ⚠️ 待修正 (${unassigned.length})</h3><div>${unassigned.map(t=>{const idx=tasks.indexOf(t);return `<div class="card" style="display:flex;align-items:center;gap:8px"><span>${t['任務名稱']}</span><span class="edit-ctrl" style="margin-left:auto;display:flex;gap:4px"><span onclick="openModal(${idx})" style="cursor:pointer">✏️</span><span onclick="deleteTask(${idx})" style="cursor:pointer;color:var(--red)">🗑️</span></span></div>`}).join('')}</div></div>`:'';
  document.getElementById('boardView').innerHTML=`
    <div style="display:flex;flex-direction:column;gap:12px">${unCol}<div class="column" data-status="待辦" ondragover="event.preventDefault();if(_taskDragIdx!==null||_dragOwner)this.style.outline='2px dashed var(--accent)'" ondragleave="this.style.outline=''" ondrop="this.style.outline='';colTaskDrop(event,'待辦')"><h3 onclick="var d=this.nextElementSibling;d.style.display=d.style.display==='none'?'block':'none';this.querySelector('.tog').textContent=d.style.display==='none'?'▶':'▼'" style="color:var(--muted);cursor:pointer"><span class="tog">▼</span> 📝 待辦 (${todo.length})</h3><div>${todo.length?groupByOwner(todo,'待辦'):'<div style="text-align:center;color:var(--muted);padding:20px">無任務</div>'}</div></div></div>
  ...
  document.querySelectorAll('#boardView .column[data-status]').forEach(col=>{const h3=col.querySelector('h3');const content=h3?.nextElementSibling;if(content&&col.querySelector('.owner-group')===null){content.style.display='none';const tog=h3.querySelector('.tog');if(tog)tog.textContent='▶'}});
...
function colTaskDrop(e,status){
  if(_taskDragIdx!==null){
    ...
    if(src['狀態']!==status){src['狀態']=status;fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:_taskDragIdx,name:src['任務名稱'],owner:src['負責人'],status:status,progress:'',startDate:src['開始日'],dueDate:src['截止日'],note:src['備註'],priority:src['優先級'],tags:src['標籤'],parent:src['父任務'],hours:src['工時'],comment:src['評論']}),})}
...
function taskDrop(e,targetIdx,el){
  ...
  if(src['狀態']!==targetStatus){
    src['狀態']=targetStatus;
    fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:_taskDragIdx,name:src['任務名稱'],owner:src['負責人'],status:targetStatus,progress:'',startDate:src['開始日'],dueDate:src['截止日'],note:src['備註'],priority:src['優先級'],tags:src['標籤'],parent:src['父任務'],hours:src['工時'],comment:src['評論']}),});
  }
  ...
  sameStatus.forEach((item,i)=>{item['排序']=String(i+1);const idx=tasks.indexOf(item);fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'updateSort',row:idx,sort:i+1}),})});
  taskDragEnd();render();renderFilterBar();
}
...
function ownerDropZone(e,targetStatus){
  ...
  filtered.filter(t=>(t['負責人']||'未指派')===owner&&!t['父任務']&&t['狀態']===_dragOwnerSrcStatus).forEach(t=>{
    ...
    fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:idx,name:t['任務名稱'],owner:t['負責人'],status:targetStatus,progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']}),});
  });
...
function renderTimeline(){
  ...
  if(!tv._ganttDelegated){tv._ganttDelegated=true;tv.addEventListener('mousedown',e=>{
    ...
    if(handle){
      ...
      const onUp=()=>{document.removeEventListener('mousemove',onMove);document.removeEventListener('mouseup',onUp);const t=tasks[idx];fetch(SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'update',row:idx,name:t['任務名稱'],owner:t['負責人'],status:t['狀態'],progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']}),})};
...
function outsourceDrop(e,zone){
  ...
  localStorage.setItem('fzg_outsource_zones',JSON.stringify(outsourceZones));
  fetch(OUTSOURCE_SCRIPT_URL,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'saveZone',owner:owner,zone:outsourceZones[owner]||zone,sort:outsourceZones['_sort_'+owner]||''}),mode:'cors'}).catch(()=>{});
...
function renderOutsourceFromCache(){
  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    const groups2=[];
    items.forEach(t=>{const id=gid(t);const mk=_manualBoardGroups[id];if(mk==='__independent__'){groups2.push([t]);return}if(mk){const g=groups2.find(gr=>gr.key===mk);if(g){g.push(t)}else{const ng=[t];ng.key=mk;groups2.push(ng)}}else{const g=groups2.find(gr=>!gr.key&&sim(gr[0]['工作項目'],t['工作項目'])&&_manualBoardGroups[gid(gr[0])]!=='__independent__');if(g)g.push(t);else groups2.push([t])}});
