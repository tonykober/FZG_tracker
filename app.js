...
function editGroupName(el,key){if(!unlocked)return;const current=el.textContent;const input=document.createElement('input');input.value=current;input.style.cssText='width:100px;font-size:inherit;background:var(--bg);border:1px solid var(--accent);border-radius:4px;padding:2px 4px;color:var(--text)';input.onblur=()=>{const v=input.value.trim()||current;_groupNames[key]=v;localStorage.setItem(getGroupNamesKey(),JSON.stringify(_groupNames));fetch(SCRIPT_URL,{method:'POST',body:JSON.stringify({action:'saveNote',month:'group_names_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),text:JSON.stringify(_groupNames)}),mode:'no-cors'});el.textContent=v;el.style.display='';input.remove()};input.onkeydown=e=>{if(e.key==='Enter')input.blur()};el.style.display='none';el.parentNode.insertBefore(input,el.nextSibling);input.focus();input.select()}
  ...
  Object.entries(owners).sort((a,b)=>(parseInt(zones['_sort_'+a[0]])||999)-(parseInt(zones['_sort_'+b[0]])||999)).forEach(([owner,items],i)=>{
    ...
    groups2.forEach(gr=>{
      if(gr.length===1){
        ...
        c+=`<div class="card" style="cursor:default"><div class="name" data-board-group="${grKey.replace(/"/g,'&quot;')}" onclick="toggleOutsourceBoardGroup(this)" style="cursor:pointer"><span>${_collapsedOutsourceBoardGroups.has(grKey)?'▼':'▶'}</span> 📁 <span onclick="event.stopPropagation();editGroupName(this,'${grKey.replace(/'/g,"\\'")}')" style="cursor:text">${grName}</span> (${gr.length})</div><div${_collapsedOutsourceBoardGroups.has(grKey)?'':' style="display:none"'}>`;
