...
function renderOutsourceFromCache(){
  ...
  if(outsourceMode==='gantt'){
    ...
    Object.entries(gGroups).forEach(([owner,items])=>{
      gh+=`<div style="border-bottom:1px solid var(--border);padding:4px 0"><span data-owner="${owner}" onclick="toggleTimelineGroup(this)" style="cursor:pointer;font-size:1rem;color:var(--accent);font-weight:600;display:inline-flex;align-items:center;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><span class="tog">${_collapsedTimelineOwners.has(owner)?'▶':'▼'}</span>&nbsp;👤 ${owner} (${items.length})</span><div${_collapsedTimelineOwners.has(owner)?' style="display:none"':''}>`;
      items.forEach(t=>{
