...
function renderBoard(){
  ...
  let warnHtml='';
  if(unassigned.length){warnHtml=`<div style="background:var(--bg);border:1px solid var(--yellow);border-radius:6px;padding:8px 12px;margin-bottom:12px;color:var(--yellow)"><span onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'block':'none'" style="cursor:pointer">⚠️ 有 ${unassigned.length} 筆任務未指派負責人</span><div style="display:none;margin-top:8px">${unassigned.map((t,i)=>{const idx=tasks.indexOf(t);return`<div style="display:flex;align-items:center;gap:8px;padding:2px 0"><span style="color:var(--fg)">${t['任務名稱']}</span><span class="edit-ctrl" onclick="deleteTask(${idx})" style="cursor:pointer;color:var(--red)">🗑️</span></div>`}).join('')}</div></div>`}
  ...
  document.getElementById('boardWarn').innerHTML=warnHtml;
