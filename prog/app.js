...
function toggleColumn(el){var d=el.nextElementSibling;var collapsed=d.style.display!=='none';d.style.display=collapsed?'none':'block';el.querySelector('.tog').textContent=collapsed?'▶':'▼';var status=el.closest('.column')?.dataset.status;if(status){var key='fzg_col_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1);var list=JSON.parse(localStorage.getItem(key)||'[]');if(collapsed){if(!list.includes(status))list.push(status)}else{list=list.filter(s=>s!==status)}localStorage.setItem(key,JSON.stringify(list));saveNote('col_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),JSON.stringify(list))}}
...
function renderBoard(){
  ...
  document.getElementById('boardView').innerHTML=`
    ...
    <div class="column" data-status="已完成" ondragover="event.preventDefault();if(_taskDragIdx!==null||_dragOwner)this.style.outline='2px dashed var(--accent)'" ondragleave="this.style.outline=''" ondrop="this.style.outline='';colTaskDrop(event,'已完成')"><h3 onclick="toggleColumn(this)" style="color:var(--green);cursor:pointer"><span class="tog">▼</span> ✅ 已完成 (${done.length})</h3><div>${done.length?groupByOwner(done,'已完成'):'<div style="text-align:center;color:var(--muted);padding:20px">無任務</div>'}</div></div>`;
