...
const SHEET_ID='1yZLzOIcNLYadjGhyZZ7LsFsfcaV1A1gFmLLBIYuGLwU';
...
const SCRIPT_URL='https://script.google.com/macros/s/AKfycbxrEYSY5jCnYXbrBY-1or5VGGSe1serugCHqWYtPU8nGC_zQcUg_nBFR2OcjqscyJs6CQ/exec';
...
function inlineEdit(idx,field,e){
  ...
  m.querySelector('#ie-ok').onclick=()=>{
    ...
    setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',row:idx,name:t['任務名稱'],owner:t['負責人'],status:t['狀態'],progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']}),});
    m.remove();render();
  };
...
async function submitTask(){
  ...
  try{
    setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify(data),});
...
function toggleCollapse(idx,el){var b=el.closest('.card').querySelector('.card-body');var collapsed=b.style.display!=='none';b.style.display=collapsed?'none':'block';el.querySelector('span').textContent=collapsed?'▶':'▼';tasks[idx]['收合']=collapsed?'1':'';var collapsedTasks=tasks.filter(t=>t['收合']==='1').map(t=>t['任務名稱']);localStorage.setItem('fzg_task_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),JSON.stringify(collapsedTasks));saveNote('task_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1),JSON.stringify(collapsedTasks))}
...
const OUTSOURCE_SHEET_ID='1n7GrSGPfZkXScaji_h74_Vsl07e4e0apgdTfxKY6eK0';
const OUTSOURCE_SCRIPT_URL='https://script.google.com/macros/s/AKfycbyQGrEoHFDrxHRaNOFDpOUC6Rxq-4NAf0JwJnExAapoQ3rXe7AFuhBbShhBsIEur8v9hQ/exec';
