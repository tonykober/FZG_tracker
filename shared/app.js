...
function _checkCrossMonth(t){if(!t||!t['ID'])return Promise.resolve(null);const curM=currentMonth.getFullYear()+'/'+(currentMonth.getMonth()+1<10?'0':'')+(currentMonth.getMonth()+1);if(t._month===curM&&!t._crossMonth)return Promise.resolve(null);const otherMonth=t._month===curM?t._crossMonth:t._month;return new Promise(resolve=>{const overlay=document.createElement('div');overlay.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.85);display:flex;align-items:center;justify-content:center;z-index:99999';overlay.innerHTML='<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:24px;max-width:360px;width:90%;text-align:center"><h3 style="margin:0 0 12px">⚠️ 跨月任務</h3><p style="margin:0 0 20px;color:var(--muted)">此任務也存在於「<b>'+otherMonth+'</b>」月份</p><div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap"><button id="_cm-sync" style="padding:8px 16px;border:none;border-radius:6px;background:var(--accent);color:#fff;cursor:pointer;font-size:0.9rem">同步所有月份</button><button id="_cm-local" style="padding:8px 16px;border:none;border-radius:6px;background:var(--green);color:#fff;cursor:pointer;font-size:0.9rem">僅更新當月</button><button id="_cm-cancel" style="padding:8px 16px;border:none;border-radius:6px;background:var(--red);color:#fff;cursor:pointer;font-size:0.9rem">取消</button></div></div>';document.body.appendChild(overlay);overlay.querySelector('#_cm-sync').onclick=()=>{overlay.remove();resolve('sync')};overlay.querySelector('#_cm-local').onclick=()=>{overlay.remove();resolve('local')};overlay.querySelector('#_cm-cancel').onclick=()=>{overlay.remove();resolve('cancel')};overlay.onclick=(e)=>{if(e.target===overlay){overlay.remove();resolve('cancel')}}})}
...
function _moveTaskToMonth(t,row,fromMonth,toMonth){
  ...
  fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'add',month:toMonth,name:t['任務名稱'],owner:t['負責人'],status:t['狀態'],progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註']||'',priority:t['優先級']||'',tags:t['標籤']||'',parent:t['父任務']||'',hours:t['工時']||'',comment:t['評論']||''})});
}
function deleteUnmodified(idx){if(!unlocked)return;const t=_unmodifiedTasks[idx];if(!t)return;if(!confirm('確定要刪除「'+t['任務名稱']+'」嗎？'))return;fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'delete',month:'未修正',row:idx})});_unmodifiedTasks.splice(idx,1);render()}
...
async function toggleStatus(idx,e){
  e.stopPropagation();if(!unlocked)return;
  const t=tasks[idx];
  const next=t['狀態']==='待辦'?'進行中':t['狀態']==='進行中'?'已完成':'待辦';
  const cm=await _checkCrossMonth(t);if(cm==='cancel')return;
  t['狀態']=next;
...
function inlineEdit(idx,field,e){
  ...
  m.querySelector('#ie-ok').onclick=async()=>{
    const _cm=await _checkCrossMonth(t);if(_cm==='cancel'){m.remove();return}
    ...
    _savePendingEdit(t);
    // Auto-adjust parent date if child task date exceeds parent range (recursive up)
    if(field==='日期'&&t['父任務']){
      let _cur=t;
      while(_cur['父任務']){
        const parent=tasks.find(p=>p['任務名稱']===_cur['父任務']);
        if(!parent)break;
        const children=tasks.filter(c=>c['父任務']===parent['任務名稱']);
        const dates=children.map(c=>({s:_normDate(c['開始日'])||'',e:_normDate(c['截止日'])||''})).filter(d=>d.s&&d.e);
        if(!dates.length)break;
        const minS=dates.map(d=>d.s).sort()[0];
        const maxE=dates.map(d=>d.e).sort().pop();
        let changed=false;
        if(!parent['開始日']||minS<_normDate(parent['開始日'])){parent['開始日']=minS;changed=true}
        if(!parent['截止日']||maxE>_normDate(parent['截止日'])){parent['截止日']=maxE;changed=true}
        if(!changed)break;
        ...
        fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:parent._month,row:parent._row,name:parent['任務名稱'],owner:parent['負責人'],status:parent['狀態'],progress:'',startDate:parent['開始日'],dueDate:parent['截止日'],note:parent['備註'],priority:parent['優先級'],tags:parent['標籤'],parent:parent['父任務'],hours:parent['工時'],comment:parent['評論']})}).then(()=>{
              ...
              if(_parentIsTop&&_pm2&&_pm2!==_cm){
                _moveTaskToMonth(parent,tasks.indexOf(parent),_cm,_pm2);
                // Also move children
                tasks.filter(c=>c['父任務']===parent['任務名稱']).forEach((c,ci)=>{const cIdx=tasks.indexOf(c);_moveTaskToMonth(c,cIdx,_cm,_pm2)});
              }
            });
        _cur=parent;
      }
      ...
      if(_children.length){
        ...
        _children.forEach(c=>{
          ...
          if(cChanged){
            ...
            fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:c._month,row:c._row,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:c['父任務'],hours:c['工時'],comment:c['評論']})});
          }
        });
      }
    }
    ...
    fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:t._month,row:t._row,name:t['任務名稱'],owner:t['負責人'],status:t['狀態'],progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']})}).then(()=>{
...
async function quickDelete(idx,e){
  e.stopPropagation();if(!unlocked)return;
  const t=tasks[idx];
  if(!confirm('確定要刪除「'+t['任務名稱']+'」嗎？'))return;
  const _cm=await _checkCrossMonth(t);if(_cm==='cancel')return;
  ...
  children.forEach(c=>{
    ...
    if(!parentOfDeleted){
      c['父任務']='';
      setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:c._month,row:c._row,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:'',hours:c['工時'],comment:c['評論']}),});
    } else {
      c['父任務']=parentOfDeleted;
      setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:c._month,row:c._row,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:parentOfDeleted,hours:c['工時'],comment:c['評論']}),});
    }
...
function loadNotes(){
  ...
  fetch(notesUrl).then(r=>r.text()).then(text=>{
    ...
    loadCollapsedOwners();loadTimelineCollapse();updateSyncTimestamp();render();
...
async function submitTask(){
  ...
  if(m.dataset.editIdx!==undefined){data.action='update';const _editTask=tasks[parseInt(m.dataset.editIdx)];data.row=_editTask._row;data.month=_editTask._month;
    // Close edit modal first, then show cross-month check
    m.classList.add('hidden');
    const _cm=await _checkCrossMonth(_editTask);
    if(_cm==='cancel'){m.classList.remove('hidden');return}
  ...
  try{
    ...
    if(!data.action){data.action='add';data.month=_curM}
    // Always send to server
    setSyncStatus('🔄 同步中...','var(--yellow)');
    fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify(data)}).then(()=>{
      if(data.action==='update'&&_isTop&&_nm&&_nm!==data.month){
        const _t=tasks[parseInt(m.dataset.editIdx)];
        _moveTaskToMonth(_t,_t._row,data.month,_nm);
        tasks.filter(c=>c['父任務']===_t['任務名稱']).forEach(c=>{_moveTaskToMonth(c,c._row,c._month,_nm)});
      }
    });
    if(data.action==='update'){
      ...
      // Auto-expand parent date range if child exceeds (recursive up)
      if(data.parent){let _cur={['父任務']:data.parent,['開始日']:data.startDate,['截止日']:data.dueDate};while(_cur['父任務']){const parent=tasks.find(p=>p['任務名稱']===_cur['父任務']);if(!parent)break;let pc=false;const _cs=_normDate(_cur['開始日']),_ce=_normDate(_cur['截止日']),_ps=_normDate(parent['開始日']),_pe=_normDate(parent['截止日']);if(_cs&&(!_ps||_cs<_ps)){parent['開始日']=_cur['開始日'];pc=true}if(_ce&&(!_pe||_ce>_pe)){parent['截止日']=_cur['截止日'];pc=true}if(!pc)break;_savePendingEdit(parent);fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:parent._month,row:parent._row,name:parent['任務名稱'],owner:parent['負責人'],status:parent['狀態'],progress:'',startDate:parent['開始日'],dueDate:parent['截止日'],note:parent['備註'],priority:parent['優先級'],tags:parent['標籤'],parent:parent['父任務'],hours:parent['工時'],comment:parent['評論']})});_cur=parent}}
    }
...
function deleteTask(idx){
  const m=document.getElementById('addModal');
  const row=idx!==undefined?idx:parseInt(m.dataset.editIdx);
  if(row===undefined)return;
  if(!confirm('確定刪除？'))return;
  const t=tasks[row];
  setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'delete',month:t._month,row:t._row}),});
  tasks.splice(row,1);if(idx===undefined)closeModal();render();
}
async function fetchData(){
  ...
  try{
    ...
    tasks=json.table.rows.map((r,i)=>{const obj={};useCols.forEach((c,ci)=>{if(r.c&&r.c[ci])obj[c]=r.c[ci].f||String(r.c[ci].v||'');else obj[c]=''});obj._row=i;obj._month=window._unscheduledMode?'未修正':(currentMonth.getFullYear()+'/'+(currentMonth.getMonth()+1<10?'0':'')+(currentMonth.getMonth()+1));return obj}).filter(t=>t['任務名稱']);
    ...
    try{const pm=new Date(currentMonth);pm.setMonth(pm.getMonth()-1);const py=pm.getFullYear(),pmm=pm.getMonth()+1;const prevMonth=py+'/'+(pmm<10?'0'+pmm:pmm);const prevUrl=`https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:json&headers=1&sheet=${encodeURIComponent(prevMonth)}`;const r2=await fetch(prevUrl);const t2=await r2.text();const j2=JSON.parse(t2.substring(47).slice(0,-2));const c2=j2.table.cols.map(c=>c.label.trim());const prev=j2.table.rows.map((r,i)=>{const obj={};c2.forEach((c,ci)=>{if(r.c&&r.c[ci])obj[c]=r.c[ci].f||String(r.c[ci].v||'');else obj[c]=''});obj._row=i;obj._month=prevMonth;return obj}).filter(t=>{if(!t['任務名稱'])return false;const match=t['ID']?tasks.find(e=>e['ID']===t['ID']):tasks.find(e=>e['任務名稱']===t['任務名稱']);if(match){match._crossMonth=prevMonth;return false}return true});tasks=tasks.concat(prev);
...
function renderBoard(){
  ...
  const cardHtml=(items)=>items.map(t=>{
    ...
    const _cross=t._crossMonth?'<span style="font-size:0.65rem;color:var(--accent);margin-left:4px">↔'+t._crossMonth.replace(/^\d{4}\//,'')+'月</span>':'';const _crossIcon=t._crossMonth?'<span title="跨月任務：也存在於 '+t._crossMonth+'" style="position:absolute;top:-8px;left:-8px;font-size:1.2rem;z-index:1">🔗</span>':'';
    return `<div class="card" data-idx="${idx}" ondragover="taskDragOver(event,this)" ondragleave="this.classList.remove('drag-over-top','drag-over-bottom')" ondrop="taskDrop(event,${idx},this)" style="position:relative;overflow:visible;${_dbg}">${_crossIcon}
      <div class="card-buttons"><div class="name" draggable="true" ondragstart="event.stopPropagation();taskDragStart(event,${idx})" ondragend="taskDragEnd()" onclick="event.stopPropagation();toggleCollapse(${idx},this)" style="cursor:grab;flex:1"><span style="font-size:0.7em;margin-right:4px">${_taskCollapseList.includes(t['任務名稱'])?'▶':'▼'}</span>${pClass?'<span class="priority-dot '+pClass+'"></span>':''}${t['任務名稱']}${_cross}</div><span class="edit-ctrl card-btn" onclick="event.stopPropagation();openModal(${idx})" style="background:#4caf50">編輯</span>${canAddSub?`<span class="edit-ctrl card-btn" onclick="event.stopPropagation();openModalWithParent('${t['任務名稱'].replace(/'/g,"\\'")}')" style="background:var(--accent)">+子任務</span>`:''}<span class="edit-ctrl card-btn" onclick="quickDelete(${idx},event)" style="background:var(--red)">✕</span></div>
      ...
      ${children.length?'<div class="subtasks" onclick="toggleSub(this,event)" style="cursor:pointer"><span style="font-size:0.75rem">'+(JSON.parse(localStorage.getItem('fzg_sub_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1))||'[]').includes(t['任務名稱'])?'▶':'▼')+'</span> 子任務：'+childDone+'/'+children.length+'<div style="margin-top:4px'+(JSON.parse(localStorage.getItem('fzg_sub_collapse_'+currentMonth.getFullYear()+'_'+(currentMonth.getMonth()+1))||'[]').includes(t['任務名稱'])?';display:none':'')+'">'+children.map(c=>{
        ...
        const cLevel=getLevel(c);const cCanAddSub=cLevel<2;const _cdbg=getDeadlineBg(c);const _ccross=c._crossMonth?'<span title="↔'+c._crossMonth+'" style="position:absolute;top:-6px;left:-6px;font-size:1rem;z-index:1">🔗</span>':'';
        return `<div style="position:relative;overflow:visible;border:1px solid var(--border);border-radius:6px;margin-bottom:4px;${_cdbg||'background:var(--surface)'};padding:6px 8px;transition:border-color .2s" onmouseover="this.style.outline='2px solid var(--accent)';this.style.outlineOffset='-2px'" onmouseout="this.style.outline='none'">${_ccross}
  ...
  const unCol=unHtml?`<div class="column" style="border-color:var(--red)"><h3 onclick="toggleColumn(this,event)" style="color:var(--red);cursor:pointer"><span class="tog">${_showUnmodified?'▶':'▼'}</span> ⚠️ 待修正 (${unassigned.length})</h3><div${_showUnmodified?' style="display:none"':''}>${unassigned.map((t,i)=>`<div class="card" style="display:flex;align-items:center;gap:8px"><span>${t['任務名稱']}</span><span class="edit-ctrl" style="margin-left:auto;display:flex;gap:4px"><span onclick="deleteUnmodified(${i})" style="cursor:pointer;color:var(--red)">🗑️</span></span></div>`).join('')}</div></div>`:'';
...
function taskDrop(e,targetIdx,el){
  ...
  if(src['狀態']!==targetStatus){
    src['狀態']=targetStatus;
    setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:currentMonth.getFullYear()+'/'+(currentMonth.getMonth()+1<10?'0':'')+(currentMonth.getMonth()+1),row:_taskDragIdx,name:src['任務名稱'],owner:src['負責人'],status:targetStatus,progress:'',startDate:src['開始日'],dueDate:src['截止日'],note:src['備註'],priority:src['優先級'],tags:src['標籤'],parent:src['父任務'],hours:src['工時'],comment:src['評論']}),});
  }
...
async function ownerDropZone(e,targetStatus){
  ...
  const _hasCross=_affectedTasks.some(t=>t['ID']&&(t._crossMonth||t._month!==(currentMonth.getFullYear()+'/'+(currentMonth.getMonth()+1<10?'0':'')+(currentMonth.getMonth()+1))));
  let _dragSync=null;
  if(_hasCross){const cm=await _checkCrossMonth(_affectedTasks.find(t=>t._crossMonth||t._month!==(currentMonth.getFullYear()+'/'+(currentMonth.getMonth()+1<10?'0':'')+(currentMonth.getMonth()+1))));if(cm==='cancel'){ownerDragEnd();return}_dragSync=cm==='sync'}
...
function renderTimeline(){
  ...
  if(!tv._ganttDelegated){tv._ganttDelegated=true;tv.addEventListener('mousedown',e=>{
    ...
    if(handle){
      ...
      const onUp=async()=>{document.removeEventListener('mousemove',onMove);document.removeEventListener('mouseup',onUp);const t=tasks[idx];const _cm=await _checkCrossMonth(t);if(_cm==='cancel'){fetchData();return}_savePendingEdit(t);let _cur=t;while(_cur['父任務']){const parent=tasks.find(p=>p['任務名稱']===_cur['父任務']);if(!parent)break;let pc=false;const _cs=_normDate(_cur['開始日']),_ce=_normDate(_cur['截止日']),_ps=_normDate(parent['開始日']),_pe=_normDate(parent['截止日']);if(_cs&&(!_ps||_cs<_ps)){parent['開始日']=_cur['開始日'];pc=true}if(_ce&&(!_pe||_ce>_pe)){parent['截止日']=_cur['截止日'];pc=true}if(pc){_savePendingEdit(parent);fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:parent._month,row:parent._row,name:parent['任務名稱'],owner:parent['負責人'],status:parent['狀態'],progress:'',startDate:parent['開始日'],dueDate:parent['截止日'],note:parent['備註'],priority:parent['優先級'],tags:parent['標籤'],parent:parent['父任務'],hours:parent['工時'],comment:parent['評論']})})}_cur=parent}const _ch=tasks.filter(c=>c['父任務']===t['任務名稱']);_ch.forEach(c=>{let cc=false;if(t['開始日']&&c['開始日']&&c['開始日']<t['開始日']){c['開始日']=t['開始日'];cc=true}if(t['截止日']&&c['截止日']&&c['截止日']>t['截止日']){c['截止日']=t['截止日'];cc=true}if(c['開始日']&&c['截止日']&&c['開始日']>=c['截止日']){c['截止日']=c['開始日'];cc=true}if(cc){_savePendingEdit(c);fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:c._month,row:c._row,name:c['任務名稱'],owner:c['負責人'],status:c['狀態'],progress:'',startDate:c['開始日'],dueDate:c['截止日'],note:c['備註'],priority:c['優先級'],tags:c['標籤'],parent:c['父任務'],hours:c['工時'],comment:c['評論']})})}});setSyncStatus('🔄 同步中...','var(--yellow)');fetch(SCRIPT_URL,{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({action:'update',month:t._month,row:t._row,name:t['任務名稱'],owner:t['負責人'],status:t['狀態'],progress:'',startDate:t['開始日'],dueDate:t['截止日'],note:t['備註'],priority:t['優先級'],tags:t['標籤'],parent:t['父任務'],hours:t['工時'],comment:t['評論']})});if(_cm==='sync')_doSyncUpdate(t,{startDate:t['開始日'],dueDate:t['截止日']});render()};
...
function renderReport(){
  ...
  const ownerMap={};filtered.forEach(t=>{const o=t['負責人']||'未指派';if(!ownerMap[o])ownerMap[o]={total:0,done:0,doing:0,todo:0,cross:0,crossDone:0,crossDoing:0,crossTodo:0};if(t._crossMonth){ownerMap[o].cross++;if(t['狀態']==='已完成')ownerMap[o].crossDone++;else if(t['狀態']==='進行中')ownerMap[o].crossDoing++;else ownerMap[o].crossTodo++}else{ownerMap[o].total++;if(t['狀態']==='已完成')ownerMap[o].done++;else if(t['狀態']==='進行中')ownerMap[o].doing++;else ownerMap[o].todo++}});
  if(Object.keys(ownerMap).length){html+=`<div class="report-section"><h3>👤 各負責人工作數量</h3>`;Object.entries(ownerMap).forEach(([n,d])=>{html+=`<p style="margin-bottom:4px">• <strong>${n}</strong>：共${d.total}項（<span style="color:var(--muted)">待辦${d.todo}</span>/<span style="color:var(--yellow)">進行中${d.doing}</span>/<span style="color:var(--green)">完成${d.done}</span>）${d.cross?'<span style="color:var(--accent);margin-left:6px">🔗 跨月'+d.cross+'項（<span style="color:var(--muted)">待辦'+d.crossTodo+'</span>/<span style="color:var(--yellow)">進行中'+d.crossDoing+'</span>/<span style="color:var(--green)">完成'+d.crossDone+'</span>）</span>':''}</p>`});html+=`</div>`}
