...
function renderReport(){
  ...
  if(Object.keys(ownerMap).length){html+=`<div class="report-section"><h3>👤 各負責人工作數量</h3>`;Object.entries(ownerMap).forEach(([n,d])=>{html+=`<p style="margin-bottom:4px">• <strong>${n}</strong>：共${d.total}項（待辦${d.todo}/進行中${d.doing}/完成${d.done}）${d.cross?'<span style="color:var(--accent);margin-left:6px">🔗 跨月'+d.cross+'項（待辦'+d.crossTodo+'/進行中'+d.crossDoing+'/完成'+d.crossDone+'）</span>':''}</p>`});html+=`</div>`}
