...
function renderBoard(){
  ...
  const isComplete=t=>t['負責人']&&t['開始日']&&t['截止日'];
  const todo=parentTasks.filter(t=>t['狀態']==='待辦'&&isComplete(t)).sort(sortFn);
  const doing=parentTasks.filter(t=>t['狀態']==='進行中'&&isComplete(t)).sort(sortFn);
  const done=parentTasks.filter(t=>t['狀態']==='已完成'&&isComplete(t)).sort(sortFn);
